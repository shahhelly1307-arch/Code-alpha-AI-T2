...
# --- 2. INTRO PAGE (HTML/CSS + IMPROVED VOICE) ---
if not st.session_state.intro_done:
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .start-btn:hover { background: #5ce1ff; color: black; box-shadow: 0 0 20px #5ce1ff; }
            ...
            @keyframes eyeMovement { 0%, 10%, 100% { transform: translate(0, 0); } 20%, 40% { transform: translate(3px, -1px); } 50%, 70% { transform: translate(-3px, 1px); } }
    ...
    <div class="container" id="main-cont">
        ...
        <button class="start-btn" id="voiceBtn">INITIALIZE NEURAL LINK</button>
        <p id="status" style="font-size: 0.7rem; margin-top: 10px; opacity: 0.5;">(Click button to hear voice greeting)</p>
    </div>
    ...
    <script>
        ...
        function speak() {
            if ('speechSynthesis' in window) {
                ...
                // Find a female voice specifically
                let femaleVoice = voices.find(v => v.name.toLowerCase().includes('female') || v.name.toLowerCase().includes('zira') || v.name.toLowerCase().includes('samantha') || v.name.toLowerCase().includes('google uk english female'));
                
                if (femaleVoice) msg.voice = femaleVoice;
                window.speechSynthesis.speak(msg);
                
                btn.style.display = 'none';
                status.innerText = "LINK ESTABLISHED. LOADING INTERFACE...";
            }
        }
    ...
    # Increased wait time so user has time to click the button and hear the voice
    time.sleep(5.0) 
    ...
    st.markdown("""
        ...
        .voxa-header {
            ...
            margin-top: 10px; filter: drop-shadow(0 0 15px rgba(0, 229, 255, 0.4));
        }
        ...
        div.stButton > button {
            ...
            border: 1px solid rgba(0, 229, 255, 0.5) !important; border-radius: 50px !important;
            font-size: 0.85rem !important; transition: 0.3s; padding: 10px 20px;
        }
